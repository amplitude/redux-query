...
const useMemoizedQueryConfigs = (
    ...
    transform: (
        arg0: QueryConfig | null | undefined,
    ) => QueryConfig | null | undefined = identity,
) => {
    const queryConfigs = providedQueryConfigs
        ? providedQueryConfigs
              .map((queryConfig: QueryConfig | null | undefined):
                  | QueryConfig
                  | null
                  | undefined => {
              ...
              .filter(Boolean)
        : []
    const [memoizedQueryConfigs, setMemoizedQueryConfigs] = React.useState(
        queryConfigs,
    )
    ...
    React.useEffect(() => {
        ...
        if (
            ...
            queryKeys.some(
                (queryKey, i) => previousQueryKeys.current[i] !== queryKey,
            )
        ) {
