...
import {
    ...
    mutateFailure,
    mutateSuccess,
Action, PublicAction} from "../actions"
...
const queryMiddleware = (
    ...
    return ({dispatch, getState}: ReduxStore) => (next: Next) => (
        action: PublicAction,
    ) => {
        let returnValue
                ...
                if (
                    ...
                    returnValue = new Promise<ActionPromiseValue>((resolve) => {
                        ...
                        const attemptRequest = () => {
                            const networkHandler = networkInterface(
                                url,
                                method,
                                {
                                    body,
                            ...
                            networkHandler.execute(
                                (
                                    ...
                                    if (
                                        config.retryableStatusCodes.includes(
                                            status,
                                        ) &&
                                    ...
                                    if (err || !isStatusOk(status)) {
                                        ...
                                        const entities = entitiesSelector(
                                            callbackState,
                                        )
                            ...
                            if (err || !isStatusOk(status)) {
                                ...
                                if (optimisticUpdate) {
                                    rolledBackEntities = rollbackEntities(
                                        ...
                                        pick(
                                            initialEntities,
                                            Object.keys(optimisticEntities),
                                        ),
                                        pick(
                                            entities,
                                            Object.keys(optimisticEntities),
