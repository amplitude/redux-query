...
const connectRequest = (mapPropsToConfigs, options = {}) =>
  WrappedComponent => {
    ...
    class ConnectRequest extends React.Component {
      ...
      componentDidUpdate(prevProps) {
        const prevConfigs = ensureArray(mapPropsToConfigs(prevProps)).filter(
          Boolean
        );
        const configs = ensureArray(mapPropsToConfigs(this.props)).filter(
          Boolean
        );
      ...
      render() {
        const configs = mapPropsToConfigs(this.props);
        const forceRequest = partial(
          this.requestAsync.bind(this),
          configs,
          true,
          false
        );
        ...
        if (withRef) {
          ...
          return (
            <WrappedComponent {...this.props} forceRequest={forceRequest} />
          );
        }
      }
    }
